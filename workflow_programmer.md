...
- **H10 Basic Data + sorting (user 2026-07-26)** — no separate Phase 5 needed: the H10 tab is filled from the **Phase-3 Xray uploads** (which already carry every needed column). `run_phase4(xray_files=…)` → `_build_per_kw` → `write_critical_sheet(xray_by_kw=…)` → `_h10_cells`: 3 keyword sections (KW1 rows 8-64, KW2 71-127, KW3 132-188; date B5/B68/B129, keyword B6/B69/B130, Amazon URL M3/M4/M5), top-57 products per keyword by `asin_sales` desc, cols A-O (title/asin/brand/price/asin_sales/asin_revenue/bsr/fees/active_sellers/rating/review_count/images/review_velocity/buy_box/category). Web view passes `media/phase3_xray/<asin>/`. The Critical sheet `rows` are now **sorted by monthly sales descending** (flat sort, Unknown→bottom) before writing. The surgical writer now edits multiple parts via an `edited` dict (Critical + Top Relevant ASINs + H10). Sample deliverable: `output/Phase4_SAMPLE_output.xlsx`.
